...
export const baseURL = "http://charlottecoffe.ga:8080/";
...
const http = {
  ...
  ) => instance.get<R, ResultModel<R>>(id ? `${url}/${id}` : url, config),
  post: <R = any>(url: string, data: any, config?: AxiosRequestConfig<any>) =>
    instance.post<R, ResultModel<R>>(url, data, config),
