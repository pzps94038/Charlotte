...
export class RouterSettingComponent
  extends BaseDataTable<Router>
  implements OnInit, InitDataTableFunction<Router>, OnDestroy
{
  destroy$ = new Subject();
  constructor(
  ...
  modify(row: Router): void {
    ...
    dialog
      ...
      .pipe(
        filter((data) => !this.sharedService.isNullorEmpty(data)),
        concatMap((data) =>
          this.routerService.modifyRouter(row.routerId, data)
        ),
        filter((res) => this.apiService.judgeSuccess(res, true)),
        takeUntil(this.destroy$)
      )
  ...
  multipleDelete(rows: Router[]): void {
    this.swalService
      ...
      .pipe(
        ...
        concatMap((deleteArr) =>
          this.routerService.batchDeleteRouter(deleteArr)
        ),
        filter((res) => this.apiService.judgeSuccess(res, true)),
        takeUntil(this.destroy$)
      )
