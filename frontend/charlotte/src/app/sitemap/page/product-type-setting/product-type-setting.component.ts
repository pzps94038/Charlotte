...
import { BehaviorSubject, concatMap, filter, map, Subject, takeUntil, finalize } from 'rxjs';
...
export class ProductTypeSettingComponent extends BaseDataTable<ProductType> implements OnInit,  InitDataTableFunction<ProductType>, OnDestroy {
  ...
  getProductTypes(info?: DataTableInfo){
    ...
    this.productTypeService.getProductTypes(info).pipe(
  ...
  modify(row: ProductType): void {
    ...
    dialog.afterClosed().pipe(
      filter(data=> !this.sharedService.isNullorEmpty(data)),
      concatMap(data=> this.productTypeService.modifyProductType(row.productTypeId, data)),
      filter(res=> this.apiService.judgeSuccess(res, true)),
      takeUntil(this.destroy$)
    ).subscribe(()=>{
      this.refresh()
  ...
  delete(row: ProductType): void {
    this.swalService.delete().pipe(
      ...
      concatMap(()=> this.productTypeService.deleteProductType(row.productTypeId)),
      filter(res=> this.apiService.judgeSuccess(res, true)),
      takeUntil(this.destroy$)
    ).subscribe(()=>{
      this.refresh()
  ...
  multipleDelete(rows: ProductType[]): void {
    this.swalService.multipleDelete(rows).pipe(
      ...
      concatMap((deleteArr)=> this.productTypeService.batchDeleteProductType(deleteArr)),
      filter(res=> this.apiService.judgeSuccess(res, true)),
      takeUntil(this.destroy$)
    ).subscribe(()=>{
      this.refresh()
