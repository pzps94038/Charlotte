...
import {
  ChceckRoleAuthResult,
  CreateRoleRequest,
  GetRoleAuthResult,
  GetRoleResult,
  ModifyRoleAuthRequest,
  ModifyRoleRequest,
} from './role.interface';
...
export class RoleService {
  ...
  getRoles(
    info?: DataTableInfo
  ): Observable<ResultModel<DataTable<GetRoleResult>>> {
  ...
  getRoleAuth(roleId: number): Observable<ResultModel<GetRoleAuthResult[]>> {
    return this.http.get<ResultModel<GetRoleAuthResult[]>>(
      `${ApiUrl.roleAuth}\\${roleId}`
    );
  }

  modifyRoleAuth(
    roleId: number,
    req: ModifyRoleAuthRequest[]
  ...
  checkRoleAuth(
    userId: number,
    routerPath: string
  ): Observable<ResultModel<ChceckRoleAuthResult>> {
